...
@testset "Solver Tests" begin
    ...
    @testset "Core Functions" begin
        @test ω(1.0, 0.0, 1.0) ≈ exp(1.0)
        @test ω(1.0, 1.0, 1.0) ≈ 1.0
        @test ω(0.0, 1.0, 0.5) ≈ exp(-2.0)

        @test ξ(1.0, 0.0, 1.0) ≈ -1.0
        @test ξ(0.0, 1.0, 1.0) ≈ 0.0
        @test ξ(2.0, 3.0, 2) ≈ 4.0

        @test β(1.0, 1.0, 0.0, 1.0) ≈ 0.0
        @test β(1.0, 1.0, 1.0, 1.0) ≈ 0.5
        @test β(2.0, 3.5, 0.56, 5.0) ≈ 0.0014622165143
    end

    @testset "Control Smoothing" begin
        ...
        τ_smooth = 1.0
        c = [0.0]
        smooth_control!(c, t, control_t, current, previous, τ_smooth)
        @test c ≈ [1.5]
    ...
    @testset "Long Integration" begin
        for T in [Float32, Float64]
            ...
            solve_pde!(prob_fd)
            @test prob_fd.sol.retcode == :Success

            # Test pseudospectral method
            prob_ps = RDEProblem(RDEParam{T}(N=512, tmax=5.0), 
                               method=PseudospectralMethod{T}())
            solve_pde!(prob_ps)
            @test prob_ps.sol.retcode == :Success
        end
    end
