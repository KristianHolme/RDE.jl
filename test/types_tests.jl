...
@testset "Types and Initialization" begin
    ...
    @testset "Method Types" begin
        ...
        @test ps_method.dealias == true
        @test ps_method.cache === nothing
        @test sprint(show, ps_method) == "PseudospectralMethod{Float32} (with dealiasing, cache uninitialized)"

        ps_method_no_dealias = PseudospectralMethod{Float32}(dealias=false)
        @test ps_method_no_dealias.dealias == false
        @test ps_method_no_dealias.cache === nothing
        @test sprint(show, ps_method_no_dealias) == "PseudospectralMethod{Float32} (without dealiasing, cache uninitialized)"
    ...
    @testset "Cache Initialization" begin
        ...
        ps_method = PseudospectralMethod{Float32}()
        init_cache!(ps_method, params, dx)
        @test ps_method.cache isa PseudospectralRDECache{Float32}
        @test length(ps_method.cache.u_x) == params.N
        @test length(ps_method.cache.dealias_filter) == div(params.N, 2) + 1
        @test sprint(show, ps_method) == "PseudospectralMethod{Float32} (with dealiasing, cache initialized)"

        # Test FiniteDifferenceMethod cache
        fd_method = FiniteDifferenceMethod{Float32}()
        init_cache!(fd_method, params, dx)
        @test fd_method.cache isa FDRDECache{Float32}
    ...
    @testset "Problem Construction" begin
        ...
        @test prob.method.cache isa FDRDECache{Float32}
        ...
        @test prob_ps.method.cache isa PseudospectralRDECache{Float32}
    end
end
